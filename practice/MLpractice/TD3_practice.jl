...
import ReinforcementLearningEnvironments:
    PendulumEnv, state, act!, reset!, is_terminated, reward
import Random
using Functors
import Zygote: Grads

actor_net() = ContinuousNetwork(
    Chain(Dense(3, 64, relu), Dense(64, 64, relu), Dense(64, 1, tanh), x -> 2.0f0 * x),
    1,
)

critic_net() =
    ContinuousNetwork(Chain(Dense(4, 64, relu), Dense(64, 64, relu), Dense(64, 1)))
...
S = ContinuousSpace(3; σ = [1.0f0, 1.0f0, 1.0f0])
...
solver = TD3(;
    π = Crux.ActorCritic(actor_net(), DoubleNetwork(critic_net(), critic_net())),
    S = S,
    off_policy...,
)
